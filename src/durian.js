...
export default function Durian({ session }) {
  ...
  const [data, setData] = useState([]); // เพิ่มตัวแปร state
  ...
  const fetchDurianData = async () => {
    ...
    try {
      ...
      console.error("Error fetching durian data:", error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={`app-container ${theme}`} style={{ position: "relative", padding: "20px" }}>
      ...
      <h1 className="theme-text" style={{ textAlign: "center" }}>Durian Data</h1>
